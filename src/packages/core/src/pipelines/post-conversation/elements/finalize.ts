...
const GUIDANCE_CONTINUE = `（系统提示：前次回复可能未完全满足用户需求。请直接补充用户实际需要的关键信息，不要重复已说过的内容。）`;
...
export class PostConversationFinalizeElement extends BaseElement<PostConversationFlowState, PipelineResult> {
  ...
  async doProcess(input: PostConversationFlowState): Promise<PipelineResult> {
    ...
    if (status === "satisfactory") {
      return { type: "complete", task: input.task, output: `post-conversation: satisfactory — ${reason}` };
    }
    ...
    if (status === "incomplete") {
      input.session.postCheckGuidance = GUIDANCE_CONTINUE;
      this.report(BusEvents.Conversation.Chain, {
        sessionId: input.session.sessionId,
        chatId: input.task?.chatId ?? "",
        parentTaskId: input.task?.parentTaskId ?? input.task?.id ?? "",
      ...
      return { type: "complete", task: input.task, output: `post-conversation: incomplete, scheduling continuation — ${reason}` };
    }
