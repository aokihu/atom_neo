...
- Existing memory is confirmed wrong, outdated, or the user explicitly asks to delete it → call \`forget_memory\`; its argument must be a full or short ID from \`<MemorySummary id="...">\` or \`<Memory id="...">\`, never memory content.
...
- Automatic Memory search and \`search_memory\` return summaries only. If a summary is relevant, call \`read_memory\` for the full content; do not treat a summary as fact or use network tools before reading it.
