...
**模式检测**：编译后 `import.meta.path` 指向编译时的源码路径（目标机器上不存在），使用 `Bun.file(import.meta.path).exists()` 判断当前是开发还是编译模式。

### 3.2 主进程侧 (`src/bootstrap/first-run.ts`)

```typescript
// 纯 Bun-native，无需 node:fs
...
async function spawnWizard(sandboxPath: string): Promise<Bun.Subprocess> {
  const isDev = await Bun.file(import.meta.path).exists();
...
```typescript
// 在 onComplete 回调中执行
async function commit(sandboxPath: string, state: WizardState): Promise<void> {
  const sandbox = sandboxPath;

  // 1. 写入 config.json（合并到已有配置）
  const configFile = Bun.file(`${sandbox}/config.json`);
  const existingConfig = (await configFile.exists())
    ? JSON.parse(await configFile.text())
    : {};
  const merged = { version: 2, ...existingConfig, ...buildConfig(state) };
  await Bun.write(`${sandbox}/config.json`, JSON.stringify(merged, null, 2));

  // 2. 追加 .env（不覆盖已有变量）
  const envPath = `${sandbox}/.env`;
  const envFile = Bun.file(envPath);
  const envLine = `${state.apiKeyEnv}=${state.apiKey}\n`;
  const existingEnv = (await envFile.exists())
    ? await envFile.text()
    : "";
  // 仅追加，不覆盖已有同名变量
  ...
  if (state.projectDescription) {
    await Bun.write(`${sandbox}/AGENTS.md`, buildAgentsMd(state.projectDescription));
  }

  // 4. 创建 .atom/installed 标记
  await Bun.write(`${sandbox}/.atom/installed`, "");
}
```
...
| 文档 | 说明 |
|------|------|
| [bootstrap.md](../overview/bootstrap.md) | 启动序列中 first-run 检测的位置 |
| [sandbox.md](./sandbox.md) | `.atom/installed` 标记文件和沙箱目录结构 |
| [configuration.md](./configuration.md) | 向导输出的 config.json 结构 |
